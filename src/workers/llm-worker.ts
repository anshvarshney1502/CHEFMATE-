    ...
    try {
      const result = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(() => {
            timedOut = true;
            reject(new Error('Generation is taking too long. Please try again.'));
          }, 60_000)
        ),
      ]);
      post({ type: 'done', id: msg.id, text: result.text?.trim() ?? '' });
    } catch (err: any) {
      // needsRestart=true tells the bridge to terminate and restart this worker.
      // Without this, the stuck WASM inference keeps running and new generate()
      // calls race against it, causing crashes on every subsequent query.
      post({ type: 'error', id: msg.id, error: err?.message ?? String(err), needsRestart: timedOut });
    } finally {
      // Keep isGenerating=true if timed out — worker will be restarted by bridge.
      // Resetting it would allow a second inference to race the stuck WASM.
      if (!timedOut) isGenerating = false;
    }
  }
};
